...
describe('appwrite-admin.listUsers - cursor handling', () => {
  ...
  it('handles malformed cursor gracefully (falls back to offset 0)', async () => {
    // Spy getUsersService to return a stubbed users service
    vi.spyOn(adminHelper, 'getUsersService').mockImplementation(() => {
      return {
  ...
  const result = await adminHelper.listUsers({ limit: 10, cursor: 'not-a-base64-token', q: undefined })
